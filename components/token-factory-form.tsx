...
export function TokenFactoryForm() {
  ...
  const [formData, setFormData] = useState<TokenDeploymentData>({
    ...
    totalSupply: "",
    description: "",
    paymentAmount: creationFee?.toString() ?? "0.01", // Default to 0.01 if creationFee is not available
  });
  ...
  useEffect(() => {
    if (creationFee) {
      setFormData((prev) => ({
        ...prev,
        paymentAmount: creationFee.toString(),
      }));
    }
  }, [creationFee]);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.paymentAmount.trim()) {
      ...
      Number(formData.paymentAmount) < 0.01
    ) {
  ...
  const handleInputChange = (
    field: keyof TokenDeploymentData,
    value: string
  ) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 px-4 sm:px-0">
      {/* Mainnet Warning */}
      {chain?.id &&
        isMainnetChain(chain.id) &&
        !isMainnetContractReadyByChainId(chain.id) && (
          <Alert className="border-orange-200 bg-orange-50">
            ...
            <AlertDescription className="text-orange-800 text-sm sm:text-base">
              <strong>Development Mode:</strong> Mainnet contract address is not
              yet configured. This is for development and testing purposes only.
              The contract address will be updated when the mainnet deployment
              is ready.
            </AlertDescription>
          </Alert>
        )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    The full name of your token (e.g., "Bitcoin")
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-500">
                    Short identifier for your token (e.g., "BTC")
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-gray-500">
                  Total number of tokens that will be created and sent to your
                  wallet
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                />
                <p className="text-xs text-gray-500">
                  Help others understand what your token is for
                </p>
              </div>
            </div>
            ...
            <div className="border-t pt-4 sm:pt-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  step="0.01"
                  min="0.01"
                  placeholder={formData.paymentAmount}
                ...
                <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                  <p className="text-xs sm:text-sm text-green-800">
                    <strong>
                      Minimum Required: {formData.paymentAmount.toString()}{" "}
                      {getCurrentNetworkConfig().currency.symbol}
                    </strong>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                          <code className="text-xs bg-green-100 px-2 py-1 rounded font-mono break-all sm:break-normal">
                            <span className="sm:hidden">
                              {deployedTokenAddress.slice(0, 8)}...
                              {deployedTokenAddress.slice(-6)}
                            </span>
                            <span className="hidden sm:inline">
                              {deployedTokenAddress.slice(0, 10)}...
                              {deployedTokenAddress.slice(-8)}
                            </span>
                          </code>
                          <Button
                            variant="ghost"
                      ...
                      <p className="text-xs sm:text-sm">
                        Your {formData.name} ({formData.symbol}) tokens have
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  <span className="sm:hidden">
                    Deploy ({formData.paymentAmount} VRCN)
                  </span>
                </>
              )}
            </Button>
